...
parser = OptionParser()
parser.add_option("-l", "--lookupfile", action="store", type="string", dest="lookupFile",
                  default="latin_arabic_character_lookup.txt", help="latin/arabic lookup filename")
parser.add_option("-p", "--primarychars", action="store_true", dest="primaryChars", default=False,
                  help="use primary characters only?")
parser.add_option("-u", "--unicode", action="store_true", dest="unicode", default=False,
                  help="use unicode characters from town names?")
parser.add_option("-w", "--wholewords", action="store_true", dest="wholeWords", default=False,
                  help="whole word targets?")
parser.add_option("-d", "--dictfile", action="store", type="string", dest="dictFile", default="latin_dict.txt",
                  help="dict file (for whole word targets)")
parser.add_option("-b", "--backwards", action="store_true", dest="backwards", default=False,
                  help="feed in targets backwards?")
...
if options.unicode:
    ...
    # read in labels [read all lines of "latin arabic character lookup" text files]
    labels = []
...
# read in input files [read all images files]
...
for f in filenames:
    ...
    if fname <> "":
        ...
        # read in transcription file of the corresponding image
        ...
        try:
            ...
            for l in file(truFilename).readlines():
                words = l.split()
                # get the line that contain "LBL" because it has the pronunciation of the image
                if words[0] == 'LBL:':
                    data = words[1].split(';')
                    ...
                    if options.wholeWords:
                        ...
                        for match in re.finditer("[0-9]{2,}", transcript):
                            transcriptNumFlip += transcript[oldStart:match.start()] + transcript[
...
# Imp Step: create array of length [totLength * 1], which will be filled with your "Input Data"
